#  This file is part of the jerry's fork of myhdl library, a Python package for using
#  Python as a Hardware Description Language.
#
#  Copyright (C) 2017 Jerry Jho
#
...
""" myhdl toSystemC conversion module.
...
from myhdl import ToSystemCError, ToSystemCWarning
from myhdl._extractHierarchy import (_HierExtr, _isMem, _getMemInfo,
                                     _UserSystemCCode, _userCodeMap)
...
def _checkArgs(arglist):
    for arg in arglist:
        if not isinstance(arg, (GeneratorType, _Instantiator, _UserSystemCCode)):
            raise ToSystemCError(_error.ArgType, arg)
...
def _flatten(*args):
    ...
    for arg in args:
        if isinstance(arg, _Block):
            if arg.systemc_code is not None:
                arglist.append(arg.systemc_code)
                continue
            else:
                arg = arg.subs
        if id(arg) in _userCodeMap.get('systemc',[]):
            arglist.append(_userCodeMap['systemc'][id(arg)])
...
class _ToSystemCConvertor(object):
    ...
    def __call__(self, func, *args, **kwargs):
        ...
        if _traceSignals._tracing:
            raise ToSystemCError("Cannot use toSystemC while tracing signals")
        if not isinstance(func, _Block):
            if not callable(func):
                raise ToSystemCError(_error.FirstArgType, "got %s" % type(func))
        ...
        if isinstance(func, _Block):
            ...
            warnings.warn(
                "\n    toSystemC(): Deprecated usage: See http://dev.myhdl.org/meps/mep-114.html", stacklevel=2)
            try:
        ...
        _writeModuleHeader(vfile, intf, genlist, doc)
        _writeSigDecls(vfile, intf, siglist, memlist)
        _convertGens(genlist, vfile,intf)
        _writeModuleFooter(vfile,intf,genlist)

        vfile.close()

        # don't write testbench if module has no ports
        if len(intf.argnames) > 0 and not toSystemC.no_testbench:
...
def _writeFileHeader(f, fn, ts):
    ...
    if not toSystemC.no_myhdl_header:
        print(string.Template(myhdl_header).substitute(vars), file=f)
    if toSystemC.header:
        print(string.Template(toSystemC.header).substitute(vars), file=f)
    print(file=f)
    #print("//`timescale %s" % ts, file=f)
    print('#include "systemc.h"', file=f)
    print(file=f)


def _writeModuleHeader(f, intf, genlist, doc):
    print("SC_MODULE (%s) {" % intf.name, file=f)
    #b = StringIO()
    #for portname in intf.argnames:
    #    print("    %s;" % portname, file=b)
    #print(b.getvalue()[:-2], file=f)
    #b.close()
    #print("//end signals", file=f)
    
    #sense_tree
    clock_port = []
    for tree in genlist:
        if tree.kind == _kind.ALWAYS_SEQ:
            clock_port.append(tree.senslist[0]._toSystemC())
    ...
    for portname in intf.argnames:
        ...
        if s._name is None:
            raise ToSystemCError(_error.ShadowingSignal, portname)
        if s._inList:
            raise ToSystemCError(_error.PortInList, portname)
        ...
        if s._driven:
            if s._read:
                if not isinstance(s, _TristateSignal):
                    warnings.warn("%s: %s" % (_error.OutputPortRead, portname),
                                  category=ToSystemCWarning
                                  )
            if isinstance(s, _TristateSignal):
                print("  sc_inout <%s> %s_out;" % (r, portname), file=f)
            else:
                print("  sc_out <%s> %s_out;" % (r, portname), file=f)
            if s._driven == 'reg':
                print("  %s %s;" % (r, portname), file=f)
            else:
            ...
            if portname in clock_port:
                print("  sc_in_clk %s;" % (portname), file=f)
            else:
                print("  sc_in <%s> %s;" % (r, portname), file=f)
    print(file=f)
...
def _writeSigDecls(f, intf, siglist, memlist):
    ...
    for s in siglist:
        ...
        if s._driven:
            ...
            if not toSystemC.initial_values or k == 'wire':
                print("%s %s%s%s;" % (k, p, r, s._name), file=f)
            else:
                if isinstance(s._init, myhdl._enum.EnumItemType):
                    print("%s %s%s%s = %s;" %
                          (k, p, r, s._name, s._init._toSystemC()), file=f)
                else:
                    print("%s %s%s%s = %s;" %
                          (k, p, r, s._name, _intRepr(s._init)), file=f)
        elif s._read:
            # the original exception
            # raise ToSystemCError(_error.UndrivenSignal, s._name)
    ...
    for m in memlist:
        ...
        if m._driven:
            ...
            if toSystemC.initial_values and not k == 'wire':
    ...
    for s in constwires:
        if s._type in (bool, intbv):
            ...
            raise ToSystemCError("Unexpected type for constant signal", s._name)
        c_len = s._nrbits
        c_str = "%s" % c
    ...
    for s in siglist:
        if hasattr(s, 'toSystemC') and s._driven:
            print(s.toSystemC(), file=f)
    print(file=f)


def _writeModuleFooter(f,intf,genlist):
    clock_port = []
    for tree in genlist:
        if tree.kind == _kind.ALWAYS_SEQ:
            clock_port.append(tree.senslist[0]._toSystemC())    
    ...
    for tree in genlist:
        if tree.kind == _kind.ALWAYS_SEQ:
            sen.append(tree.senslist[0]._toSystemC() + '.' + tree.senslist[0].edge()+ '()')
            if len(tree.senslist) > 1:
                [sen.append(e._toSystemC()) for e in tree.senslist[1:]]
    print("  SC_CTOR(%s) {" % intf.name, file=f)
    for tree in genlist:
        print("    SC_METHOD(%s);" % tree.name, file=f)
    for s in sen:
        print("    sensitive<<%s;" % s, file=f)
    print("  }", file=f)
    print("};", file=f)
...
def _writeTestBench(f, intf, trace=False):
    print('#include "systemc.h"', file=f)
    print('#include "%s.cpp"' % intf.name, file=f)
    print("int sc_main (int argc, char* argv[]) {", file=f)
    print(file=f)
    fr = StringIO()
    to = StringIO()
    pm = StringIO()
    for portname in intf.argnames:
        ...
        r = _getRangeString(s)
        if s._driven:
            print("  sc_signal<%s> %s_out;" % (r, portname), file=f)
        else:
            print("  sc_signal<%s> %s;" % (r, portname), file=f)
        #if s._driven:
    
    print('  %s u_%s("%s");' % (intf.name,intf.name,intf.name), file=f)
    for portname in intf.argnames:
        s = intf.argdict[portname]
        r = _getRangeString(s)
        if s._driven:
            print("  %s.%s_out(%s_out);" % (intf.name, portname,portname), file=f)
        else:
            print("  %s.%s(%s);" % (intf.name, portname,portname), file=f)    
            
            
    print('  sc_trace_file *wf = sc_create_vcd_trace_file("%s");' % intf.name, file=f)
    for portname in intf.argnames:
        s = intf.argdict[portname]
        r = _getRangeString(s)
        if s._driven:
            print('  sc_trace(wf, %s_out, "%s_out");' % (portname,portname), file=f)
        else:
            print('  sc_trace(wf, %s, "%s");' % (portname,portname), file=f)   
    print('  #ifdef STIMULUS', file=f)
    print('    #include "stimulus.cpp"', file=f)
    print('  #endif // STIMULUS', file=f)
    print('  sc_close_vcd_trace_file(wf);', file=f)
...
def _convertGens(genlist, vfile , intf):
    blockBuf = StringIO()
    funcBuf = StringIO()
    for tree in genlist:
        if isinstance(tree, _UserSystemCCode):
...
class _ConvertVisitor(ast.NodeVisitor, _ConversionMixin):
    ...
    def __init__(self, tree, buf, intf = None):
        self.tree = tree
        self.buf = buf
    ...
    def raiseError(self, node, kind, msg=""):
        ...
        raise ToSystemCError(kind, msg, info)
    ...
    def writeAlwaysHeader(self):
        ...
        self.write("void %s {//" % self.tree.name)
    ...
    def writeSensitivityList(self, senslist):
        sep = ', '
        if toSystemC.standard == '1995':
            sep = ' or '
        self.write("@(")
        for e in senslist[:-1]:
            self.write(e._toSystemC())
            self.write(sep)
        self.write(senslist[-1]._toSystemC())
        self.write(")")
    ...
    def visit_BinOp(self, node):
        if isinstance(node.op, ast.Mod) and self.context == _context.PRINT:
            ...
            if isinstance(node.op, ast.RShift):
                ...
                # is actually required by standard SystemC.
...
#            self.writeline()
            ...
            return
        # default behavior
        systemc_late_assign = False
        self.visit(node.targets[0])
        if self.isSigAss:
            self.write(' = ')
            systemc_late_assign = True
            self.isSigAss = False
        else:
            self.write(' = ')
        self.visit(node.value)
        self.write(';')
        if systemc_late_assign:
            t = node.targets[0].value.id
            self.writeline()
            self.write('%s_out.write(%s);' % (t,t))
...
#        self.writeline()
        ...
        for test, suite in node.tests:
            ...
            if isinstance(item, EnumItemType):
                self.write(item._toSystemC(dontcare=True))
            else:
    ...
    def getName(self, node):
        ...
        self.write(s)
        if s in self.intf.argnames and not self.intf.argdict[s]._driven:
            self.write('.read()')
        if addSignBit:
            self.write("})")
...
class _ConvertAlwaysCombVisitor(_ConvertVisitor):
    ...
    def __init__(self, tree, blockBuf, funcBuf):
        _ConvertVisitor.__init__(self, tree, blockBuf)
        if toSystemC.prefer_blocking_assignments:
            self.okSigAss = False
        self.funcBuf = funcBuf
...
class _ConvertAlwaysSeqVisitor(_ConvertVisitor):
    ...
    def visit_FunctionDef(self, node):
        ...
        if reset is not None:
            self.writeline()
            self.write("if (%s.read() == %s) {" % (reset, int(reset.active)))
            self.indent()
            for s in sigregs:
                self.writeline()
                self.write("%s = %s;" % (s, _convertInitVal(s, s._init)))
                self.write("%s_out.write(%s);" % (s, _convertInitVal(s, s._init)))
            for v in varregs:
                n, reg, init = v
                self.writeline()
                self.write("%s_out.write(%s);" % (n, _convertInitVal(reg, init)))
            self.dedent()
            self.writeline()
            self.write("} else {")
            self.indent()
